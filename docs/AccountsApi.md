...
from MergeTicketingClient.api import accounts_api
...
with MergeTicketingClient.ApiClient(configuration) as api_client:
    ...
    # and optional values
    try:
        api_response = api_instance.accounts_retrieve(x_account_token, id, include_remote_data=include_remote_data)
        pprint(api_response)
    except MergeTicketingClient.ApiException as e:
        print("Exception when calling AccountsApi->accounts_retrieve: %s\n" % e)
```
